...
class _CategoryNewsState extends State<CategoryNews> {
  StreamController streamControllerCata = StreamController();

  getdata() async {
    final category = widget.category;
    Uri url = Uri.parse(
        "https://newsapi.org/v2/top-headlines?country=in&category=$category&apiKey=ad6e24f8e94741189add2767908a4d20");
    Response response = await get(url);

    if (response.statusCode == 200) {
      Map<String, dynamic> jsonData = jsonDecode(response.body);
      NewsFeed news = NewsFeed.fromJson(jsonData);
      if (!streamControllerCata.isClosed) {
        return streamControllerCata.sink.add(news);
      } else {
        return null;
      }
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          title: Text("${widget.category.toUpperCase()} Head Lines")),
      backgroundColor: Colors.grey[800],
      body: StreamBuilder(
          stream: streamControllerCata.stream,
